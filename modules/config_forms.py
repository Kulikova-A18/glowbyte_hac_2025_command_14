...
import os
import pandas as pd
from .logger import get_app_logger  # ← относительный импорт логгера
from .data_loader import load_csv
from .schedule_manager import save_schedule
...
def _setup_standard_form(category: str, file_path: str, df_preview: pd.DataFrame, default_y_cols: list):
    ...
    if col_btn1.button("Создать график"):
        try:
            ...
            st.session_state.graphs[category].append(new_chart)
            st.session_state.next_id += 1
            _persist_changes()
            logger.info(f"Создан график: категория='{category}', название='{display_name}', параметры={y_cols}")
            st.rerun()
        except Exception as e:
            error_msg = f"Ошибка при создании графика в категории '{category}': {e}"
            logger.error(error_msg, exc_info=True)
            st.error("Не удалось сохранить график. Подробности в логах.")
...
def _persist_changes():
    ...
    try:
        ...
        logger.debug("Конфигурация графиков сохранена в schedule.json")
    except Exception as e:
        error_msg = f"Ошибка при сохранении конфигурации графиков: {e}"
        logger.error(error_msg, exc_info=True)
        raise
...
def show_supplies_dialog():
    try:
        ...
        if df_preview.empty:
            ...
            logger.warning("Файл supplies.csv пуст или не загружен")
            return
        _setup_standard_form("supplies", SUPPLIES_FILE, df_preview, ["На склад, тн", "На судно, тн"])
    except Exception as e:
        error_msg = f"Ошибка в диалоге supplies: {e}"
        logger.error(error_msg, exc_info=True)
        st.error("Ошибка при загрузке данных. Подробности в логах.")
...
def show_fires_dialog():
    try:
        ...
        if df_preview.empty:
            ...
            logger.warning("Файл fires.csv пуст или не загружен")
            return
        ...
        error_msg = f"Ошибка в диалоге fires: {e}"
        logger.error(error_msg, exc_info=True)
        st.error("Ошибка при загрузке данных. Подробности в логах.")
...
def show_temperature_dialog():
    try:
        ...
        if df_preview.empty:
            ...
            logger.warning("Файл temperature.csv пуст или не загружен")
            return
        _setup_standard_form("temperature", TEMP_FILE, df_preview, ["Максимальная температура"])
    except Exception as e:
        error_msg = f"Ошибка в диалоге temperature: {e}"
        logger.error(error_msg, exc_info=True)
        st.error("Ошибка при загрузке данных. Подробности в логах.")
...
def show_weather_dialog():
    try:
        ...
        if not years:
            ...
            logger.warning("Нет файлов weather_data_*.csv с корректным годом")
            return
        ...
        if df_preview.empty:
            ...
            logger.warning(f"Файл {selected_file} пуст или не загружен")
            return
        ...
        if col_btn1.button("Создать график"):
            try:
                ...
                st.session_state.graphs["weather"].append(new_chart)
                st.session_state.next_id += 1
                _persist_changes()
                logger.info(f"Создан график погоды: год={selected_year}, название='{display_name}', параметры={y_cols}")
                st.rerun()
            except Exception as e:
                error_msg = f"Ошибка при создании графика погоды: {e}"
                logger.error(error_msg, exc_info=True)
                st.error("Не удалось сохранить график. Подробности в логах.")
        ...
        error_msg = f"Ошибка в диалоге weather: {e}"
        logger.error(error_msg, exc_info=True)
        st.error("Ошибка при настройке графика погоды. Подробности в логах.")
